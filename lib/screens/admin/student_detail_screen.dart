...
class _StudentDetailScreenState extends State<StudentDetailScreen> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  ...
                  radius: 30,
                  child: Text(
                    widget.student.fullName.isNotEmpty ? widget.student.fullName[0].toUpperCase() : '?',
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Full Name', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey)),
                      Text(widget.student.fullName, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
                IconButton(
            ...
            _buildInfoRow(Icons.school, 'University', widget.student.university),
            _buildInfoRow(Icons.location_city, 'City', '${widget.student.city.name} (Zone ${widget.student.city.zone})'),
            _buildInfoRow(Icons.calendar_today, 'Registered', DateFormat('MMM dd, yyyy').format(widget.student.createdAt)),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text('Subscription Details', style: Theme.of(context).textTheme.titleLarge),
                IconButton(
                  icon: const Icon(Icons.edit, color: Colors.blue),
            ...
            _buildInfoRow(Icons.schedule, 'Schedule', widget.student.scheduleSuffix.code),
            _buildInfoRow(Icons.description, 'Description', widget.student.scheduleSuffix.description),
            _buildInfoRow(Icons.calendar_view_week, 'Days per Week', '${widget.student.scheduleSuffix.daysPerWeek} days'),
            _buildInfoRow(Icons.attach_money, 'Cost', '\$${widget.student.subscriptionCost.toStringAsFixed(2)} / ${widget.student.subscriptionType.displayName}'),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('Selected Days & Times', style: Theme.of(context).textTheme.titleLarge),
                IconButton(
                  icon: const Icon(Icons.edit, color: Colors.blue),
  ...
  Widget _buildPaymentSummaryCards() {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: _buildSummaryCard('Total Amount', '\$${_totalAmount.toStringAsFixed(2)}', Icons.account_balance_wallet, Colors.blue),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: _buildSummaryCard('Total Paid', '\$${_totalPaid.toStringAsFixed(2)}', Icons.check_circle, Colors.green),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: _buildSummaryCard('Total Remaining', '\$${_totalRemaining.toStringAsFixed(2)}', Icons.pending, _totalRemaining > 0 ? Colors.orange : Colors.green),
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(title, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: color, fontWeight: FontWeight.w500), textAlign: TextAlign.center),
        ...
        child: DataTable(
          ...
          columns: [
            ...
            DataColumn(
              label: Row(
                children: [
                  Icon(Icons.pending_outlined, size: 16, color: Colors.grey.shade700),
                  const SizedBox(width: 4),
          ...
          rows: _paymentHistory.map((payment) {
            ...
            final isCurrentMonth = DateTime.now().year == payment.year && DateTime.now().month == payment.month;
            
            return DataRow(
              color: MaterialStateProperty.resolveWith<Color?>((states) {
                if (isCurrentMonth) return Colors.blue.shade50;
                if (payment.isFullyPaid) return Colors.green.shade50.withOpacity(0.3);
                return null;
              }),
              cells: [
                DataCell(
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              ...
                              fontSize: 14,
                              color: isCurrentMonth ? Colors.blue.shade700 : Colors.black87,
                            ),
                          ),
                          if (isCurrentMonth) ...[
                ...
                DataCell(
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(
                      color: payment.paidAmount > 0 ? Colors.green.shade50 : Colors.grey.shade100,
                      borderRadius: BorderRadius.circular(6),
                    ),
                    child: Text(
                      '\$${payment.paidAmount.toStringAsFixed(2)}',
                      style: TextStyle(
                        color: payment.paidAmount > 0 ? Colors.green.shade700 : Colors.grey.shade600,
                        fontWeight: payment.paidAmount > 0 ? FontWeight.w700 : FontWeight.w500,
                        fontSize: 13,
                      ),
                    ),
                  ),
                ...
                DataCell(
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(
                      color: payment.remainingAmount > 0 ? Colors.orange.shade50 : Colors.green.shade50,
                      borderRadius: BorderRadius.circular(6),
                    ),
                    child: Text(
                      '\$${payment.remainingAmount.toStringAsFixed(2)}',
                      style: TextStyle(
                        color: payment.remainingAmount > 0 ? Colors.orange.shade700 : Colors.green.shade700,
                      ...
                      ? Row(
                          ...
                          children: [
                            Tooltip(
                              ...
                              child: InkWell(
                                ...
                                child: Container(
                                  ...
                                  child: Icon(Icons.payment, size: 18, color: Colors.blue.shade700),
                                ),
                              ),
                            ),
                            const SizedBox(width: 8),
                            ElevatedButton.icon(
                              ...
                              label: const Text('Pay Full', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
  ...
  Future<void> _processFullPayment(MonthlyPayment payment) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Mark full payment of \$${payment.remainingAmount.toStringAsFixed(2)} for ${DateFormat('MMM yyyy').format(DateTime(payment.year, payment.month))}?'),
        actions: [
          TextButton(
  ...
  Future<void> _showPartialPaymentDialog(MonthlyPayment payment) async {
    ...
    final result = await showDialog<double>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Partial Payment - ${DateFormat('MMM yyyy').format(DateTime(payment.year, payment.month))}'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Monthly Amount: \$${payment.monthlyAmount.toStringAsFixed(2)}'),
            Text('Already Paid: \$${payment.paidAmount.toStringAsFixed(2)}'),
            Text('Remaining: \$${payment.remainingAmount.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange)),
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (amount > payment.remainingAmount) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Amount cannot exceed remaining balance of \$${payment.remainingAmount.toStringAsFixed(2)}')),
                );
                return;
              }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Partial payment of \$${result.toStringAsFixed(2)} processed successfully!'),
            backgroundColor: Colors.green,
          ),
        );
      }
  ...
  Future<void> _updateStudentPaymentStatus() async {
    ...
    final allPaid = _paymentHistory.every((payment) => payment.remainingAmount == 0);
  ...
  Future<void> _showEditStudentDialog() async {
    final fullNameController = TextEditingController(text: widget.student.fullName);
    final phoneController = TextEditingController(text: widget.student.phoneNumber);
    final universityController = TextEditingController(text: widget.student.university);
    ...
    if (widget.student.city.id.isNotEmpty) {
      ...
      try {
        selectedCity = allCities.firstWhere((city) => city.id == widget.student.city.id);
      } catch (e) {
      ...
      try {
        ...
        if (mounted) {
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => StudentDetailScreen(student: updatedProfile)),
          );
        }
      } catch (e) {
        if (mounted) {
  ...
  Future<void> _showEditSubscriptionDialog() async {
    List<ScheduleSuffix> allSchedules = await _studentService.getScheduleSuffixes();
                ...
                if (selectedSchedule != null) ...[
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Cost: \$${selectedType == SubscriptionType.monthly ? selectedSchedule!.monthlyCost : selectedSchedule!.dailyCost} / ${selectedType.displayName}', 
    ...
    if (result == true && selectedSchedule != null) {
      ...
      if (selectedSchedule!.id != widget.student.scheduleSuffix.id || 
          selectedSchedule!.daysPerWeek != widget.student.scheduleSuffix.daysPerWeek) {
        // Show days and times selection dialog
        final daysTimesResult = await _showDaysTimesSelectionDialog(selectedSchedule!.daysPerWeek);
        ...
        try {
          ...
          final updatedProfile = widget.student.copyWith(
            ...
            dayTimeSlots: daysTimesResult['dayTimeSlots'] as Map<String, TimeSlot>,
          );
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Subscription and schedule updated successfully')),
            );
            // Refresh the screen
            Navigator.pushReplacement(
              context,
  ...
  Future<Map<String, dynamic>?> _showDaysTimesSelectionDialog(int requiredDays) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text('Select $requiredDays ${requiredDays == 1 ? 'Day' : 'Days'} & ${requiredDays == 1 ? 'Time' : 'Times'}'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                ...
                ...daysOfWeek.map((day) {
                  ...
                  return Card(
                    ...
                    child: CheckboxListTile(
                      title: Text(day, style: TextStyle(fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
                      value: isSelected,
                      subtitle: isSelected && timeSlot != null 
                          ? Text(timeSlot.displayName, style: TextStyle(color: Colors.blue.shade700, fontWeight: FontWeight.w600))
                          : null,
                      onChanged: (checked) {
                        setState(() {
                          if (checked == true) {
                            if (selectedDays.length < requiredDays) {
                              ...
                              selectedDayTimeSlots[day] = TimeSlot.onepm; // Default
                            }
                          } else {
                      ...
                      secondary: isSelected 
                          ? DropdownButton<TimeSlot>(
                              ...
                              items: TimeSlot.values.map((slot) {
                                return DropdownMenuItem(
                                  value: slot,
                                  child: Text(slot.displayName, style: const TextStyle(fontSize: 12)),
                                );
                              }).toList(),
                              onChanged: (slot) {
                                setState(() {
  ...
  Future<void> _showEditDaysTimesDialog() async {
    ...
    final daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    Map<String, TimeSlot?> selectedDayTimeSlots = Map.from(widget.student.dayTimeSlots);
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text('Edit Days & Times ($requiredDays ${requiredDays == 1 ? 'Day' : 'Days'} Required)'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Container(
                  ...
                  margin: const EdgeInsets.only(bottom: 16),
                  decoration: BoxDecoration(
                    color: selectedDays.length == requiredDays ? Colors.green.shade50 : Colors.orange.shade50,
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: selectedDays.length == requiredDays ? Colors.green.shade200 : Colors.orange.shade200),
                  ),
                  child: Row(
                    children: [
                      Icon(
                        selectedDays.length == requiredDays ? Icons.check_circle : Icons.info,
                        color: selectedDays.length == requiredDays ? Colors.green.shade700 : Colors.orange.shade700,
                        size: 20,
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 12,
                            color: selectedDays.length == requiredDays ? Colors.green.shade900 : Colors.orange.shade900,
                            fontWeight: FontWeight.w600,
                          ),
                        ),
                      ),
