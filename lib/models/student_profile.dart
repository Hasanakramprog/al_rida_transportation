...
class ScheduleSuffix {
  ...
  String get displayName => '$code ($daysPerWeek ${daysPerWeek == 1 ? 'day' : 'days'} per week) - $description';
  ...
  @override
  int get hashCode => id.hashCode ^ code.hashCode ^ zone.hashCode ^ daysPerWeek.hashCode;

  @override
  String toString() => 'ScheduleSuffix(id: $id, code: $code, zone: $zone, daysPerWeek: $daysPerWeek)';
}
...
class StudentProfile {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'scheduleSuffixId': scheduleSuffix.id, // Store reference to schedule document
      'cityId': city.id,
      'cityName': city.name,
      'cityZone': city.zone,
      'subscriptionCost': subscriptionCost,
      'selectedDays': selectedDays,
      'dayTimeSlots': dayTimeSlots.map((day, timeSlot) => MapEntry(day, timeSlot.value)),
      'paymentAmount': paymentAmount,
      'isPaid': isPaid,
      'isActive': isActive,
      'paymentHistory': paymentHistory.map((history) => history.toMap()).toList(),
  ...
  factory StudentProfile.fromMap(Map<String, dynamic> map, ScheduleSuffix scheduleSuffix) {
    return StudentProfile(
      uid: map['uid'] ?? '',
      fullName: map['fullName'] ?? '',
      university: map['university'] ?? '',
      phoneNumber: map['phoneNumber'] ?? '',
      subscriptionType: SubscriptionType.fromString(map['subscriptionType'] ?? 'monthly'),
      scheduleSuffix: scheduleSuffix,
      city: City(
        id: map['cityId'] ?? '',
        name: map['cityName'] ?? '',
      ...
      dayTimeSlots: (map['dayTimeSlots'] as Map<String, dynamic>? ?? {})
          .map((key, value) => MapEntry(key, TimeSlotExtension.fromString(value ?? '1pm'))),
  ...
  PaymentHistory? getPaymentForMonth(int year, int month) {
    try {
      return paymentHistory.firstWhere((payment) => 
